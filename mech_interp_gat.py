# -*- coding: utf-8 -*-
"""Mech_interp_GAT.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sh8QgtlEmnOrK7kwOXn25BCrGTAx3OjG
"""
...
def run_causal_activation_patching(model, base_edge_index, patterns,
    ...
    For each mined pattern:
      1. Build clean edge_index (base graph -- neighbor_item is present
         in user's real history) and corrupt edge_index (neighbor_item
         swapped for a random unrelated item).
      2. Run the model on each; get per-head activations from clean and
         the corrupt-run score for target_item.
      3. For every (layer, head), splice that head's clean per-node
         activation into the corrupt run and see how much of the score
         gap it recovers.

    Note: unlike the SASRec version, we cannot batch multiple patterns
    into one forward pass, because each pattern's graph differs (a
    different user has a different neighborhood in each pattern).
    Iterate one pattern at a time.
    """
    model.eval()
    num_layers = model.num_layers
    num_heads = model.num_heads
...
def run_zero_ablation_eval(model, edge_index, eval_targets, train_seqs,
    ...
    """
    Zero out the specified head's per-node output (for ALL nodes), then
    evaluate Recall@k on the held-out targets. Passing target_layer or
    target_head outside the valid range = no ablation = baseline.
    """
...
if __name__ == "__main__":
    DATA_PATH = '/content/drive/MyDrive/Movies_and_TV.jsonl.gz'
    META_PATH = '/content/drive/MyDrive/meta_Movies_and_TV.jsonl.gz'
    HIDDEN_DIM = 64
    NUM_LAYERS = 2
    NUM_HEADS = 4
    NUM_EPOCHS = 500
